            ...
            with st.spinner('Veriler Google E-Tablolar\'a aktarılıyor...'):
                ...
                if success:
                  ...
                  # Dinamik geri sayım (countdown) mekanizması
                    ...
                    # 1. Mevcut kategorideki kullanıcı cevaplarını (dict) temizle
                    st.session_state.user_answers[selected_category] = {}
                    
                    # 2. Soru navigasyonunu sağlayan indeksi sıfırla (Reset Pointer)
                    # Not: Kodunuzda 'question_index' yerine farklı bir isim kullandıysanız (örn: q_idx) onu güncelleyin.
                    if 'question_index' in st.session_state:
                        st.session_state.question_index = 0
                    
                    # 3. İsim ve Numara giriş alanlarını temizlemek için (Varsa widget key'lerini siler)
                    if 'student_name' in st.session_state:
                        st.session_state.student_name = ""
                    if 'student_number' in st.session_state:
                        st.session_state.student_number = ""

                    # Arayüzü (Frontend) yeni bir oturum için en baştan derle
                    st.rerun()